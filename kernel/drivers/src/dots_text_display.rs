...
// Implementare default (valoare initiala ca sa nu crape) pt AppData
#[derive (Default)] 
pub struct AppData {
...
pub struct DotsTextDisplay<'a, L: Led, A: Alarm<'a>> {
    leds: &'a [&'a L; 25],
    alarm: &'a A,
    app_data: Grant<AppData, 1>, // tip si nr subscribe
    state: Cell<State>,
}
...
impl<'a, L: Led, A: Alarm<'a>> DotsTextDisplay<'a, L, A> {
    ...
    fn display_next_digit (&self) -> Result<(), ErrorCode> {
            ...
            State::Printing {
                ...
                let res = self.app_data.enter(process_id, |data, upcalls| {
                    let res = data.buffer.enter (|buffer| {
                        ...
                        if position < buffer.len() && position < len { // NECLAR
        ...
        match res {
            ...
            // DIFERENTA Ok(Err(error)) VS Err(error) !!!
            Ok(Err(error)) => {
                self.state.set(State::Idle);
                Err(error.into())
            },
        ...
        match command_num {
            ...
            1 =>  {
                if self.state.get() == State::Idle {
                    ...
                    if let Err(error) = self.display_next_digit() { // match cu o singura ramura
                    ...
                    // Se poate inlocui if - ul cu urm structura:
